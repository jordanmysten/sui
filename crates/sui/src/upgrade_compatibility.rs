...
impl UpgradeErrorList {
    ...
    fn print_errors(&mut self, compiled_unit_with_source: &CompiledUnitWithSource) {
    ...
    fn source(&mut self, compiled_unit_with_source: &CompiledUnitWithSource) -> &String {
        ...
        .map(|existing_module| {
            ...
            match new_modules_map.get(&name) {
                Some(new_module) => Compatibility::upgrade_check()
                    ...
                    .map_err(|mut e| {
                        e.print_errors(compiled_unit_with_source);
                        return anyhow!("Compatibility check failed for module '{}'", name);
                    }),
